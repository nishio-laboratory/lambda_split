...
class SplitComputingLogger(object):
    ...
    def get_yield_str(self) -> str:
        ...
        return '(Split Computing Info)\n' + \
                f'First model  : {list(range(0, self.first_split_layer_index_history[-1]))}\n' + \
                f'Second model : {list(range(self.first_split_layer_index_history[-1], self.second_split_layer_index_history[-1]))}\n' + \
                f'Third model  : {list(range(self.second_split_layer_index_history[-1], self.llm_config.num_decoder_layers))}\n' + \
